...
export function BodyMap({ onRegionSelect, selectedRegion, loggedCounts = {}, recentSeverityByRegion = {}, selectedDate }: BodyMapProps) {
  ...
  const getSeverityColor = (severity: number): { fill: string; stroke: string; glow: string } => {
    if (severity <= 3) return { fill: '#fef3c7', stroke: '#f59e0b', glow: 'rgba(245, 158, 11, 0.5)' };
    if (severity <= 6) return { fill: '#ffedd5', stroke: '#f97316', glow: 'rgba(249, 115, 22, 0.5)' };
    return { fill: '#fee2e2', stroke: '#ef4444', glow: 'rgba(239, 68, 68, 0.5)' };
  };
  ...
  const getRegionProps = (region: BodySystem) => {
    ...
    let fill: string, stroke: string, glow: string;
    if (isActive)    { fill = '#6366f1'; stroke = '#818cf8'; glow = 'rgba(99, 102, 241, 0.6)'; }
    else if (isHovered) { fill = HOVER_FILL; stroke = HOVER_STROKE; glow = 'rgba(99, 102, 241, 0.3)'; }
    ...
    else if (hasLogs){ fill = '#e0e7ff'; stroke = '#a5b4fc'; glow = 'rgba(165, 180, 252, 0.3)'; }
    else             { fill = '#f1f5f9'; stroke = '#cbd5e1'; glow = 'none'; }
    ...
    return {
      ...
      style: { 
        ...
        filter: isActive ? `drop-shadow(0 0 6px ${glow})` : isHovered ? `drop-shadow(0 0 4px ${glow})` : 'none'
  ...
  return (
    <div className="flex flex-col items-center w-full mx-auto p-5 sm:p-6 glass rounded-3xl shadow-2xl shadow-indigo-900/20 border border-white/10 animate-fade-in-up relative overflow-hidden">
      {/* Radial glow behind body map */}
      ...
      {/* Front / Back toggle */}
      <div className="flex p-1 gap-1 mb-6 glass rounded-2xl w-full">
        {(['front', 'back'] as const).map((v) => (
          <button
            key={v}
            onClick={() => setView(v)}
            className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
              view === v ? 'bg-white/15 text-indigo-200 shadow-sm backdrop-blur-sm' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {v === 'front' ? 'Front' : 'Back'}
          </button>
      ...
      {selectedDate && (
        <div className="mb-3 text-xs font-medium text-indigo-300 glass px-3 py-1.5 rounded-lg border border-indigo-400/30">
          ...
          {Object.entries(loggedCounts).map(([region, count]) => {
            ...
            return (
              <g key={region} style={{ pointerEvents: 'none' }}>
                <circle cx={cx} cy={cy} r="13" fill="#6366f1" opacity="0.95" filter="url(#glow)" />
                <circle cx={cx} cy={cy} r="13" fill="url(#badgeGradient)" opacity="0.9" />
      ...
      <div className="flex flex-wrap items-center gap-4 mt-5 text-[11px] text-slate-400 font-medium">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-orange-300 border border-orange-500 inline-block" />
          Medium (4–6)
        </span>
        <span className="flex items-center gap-1.5">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-indigo-500 border border-indigo-400 inline-block" />
          Selected
        </span>
      </div>
    </div>
