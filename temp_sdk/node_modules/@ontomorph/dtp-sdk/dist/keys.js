 ...
 */
import { DTPConfigError } from "./http.js";
...
export class KeysClient {
    ...
    requireSession() {
        if (!this.sessionToken) {
            throw new DTPConfigError("dtp.keys.* requires a user session token. Pass `sessionToken` to the DTP constructor — api-key management is user-authed, not api-key-authed.");
        }
        return this.sessionToken;
    }
    /** List the authenticated user's api keys. Maps to `GET /api-keys`. */
    async list(query) {
        return this.http.requestData(API_KEYS_PATH, {
            method: "GET",
    ...
    async revoke(id) {
        return this.http.requestData(`${API_KEYS_PATH}/${encodeURIComponent(id)}`, {
            method: "DELETE",
            auth: { bearer: this.requireSession() },
        });
    }
